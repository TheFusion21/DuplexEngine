...
// Fresnel function
float3 F_Schlick(float cosTheta, float metalness, float3 color)
{
	float3 F0 = lerp(float3(0.04, 0.04, 0.04), color, metalness); // * material.specular
	float3 F = F0 + (1.0 - F0) * pow(1.0 - cosTheta, 5.0);
	return F;
}
float3 BRDF(float3 lightColor, float3 L, float3 V, float3 N, Mat m)
{
...
float3 LightCalcDir(int i, float3 N, float3 V, Mat m, float3 L)
{
	return BRDF(lights[i].color, L, V, N, m) * lights[i].intensity;
}
float3 LightCalcSpot(int i, float3 N, float3 V, Mat m)
{
	return float3(0.0f, 0.0f, 0.0f);
}
float3 LightCalcPoint(int i, float3 N, float3 V, Mat m)
{
	return float3(0.0f, 0.0f, 0.0f);
}
...
float4 PS_Main(ps_in input) : SV_TARGET
{
	...
	for (unsigned int i = 0; i < lightCount; i++)
	{
		if (lights[i].type == 0)
		{
			Lo += LightCalcSpot(i, input.normal, view, m);
		}
		else if (lights[i].type == 1)
		{
			float3 L = normalize(mul(float4(1.0f, 0.0f, 0.0f, 1.0f), lights[i].transform)).xyz;
			Lo += LightCalcDir(i, N, view, m, L);
		}
		else if (lights[i].type == 2)
			Lo += LightCalcPoint(i, N, view, m);
	}
	color += Lo;
